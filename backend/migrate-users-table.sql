-- Migration script to add new required fields to the users table
...
-- Create an index on college for faster queries
CREATE INDEX IF NOT EXISTS idx_users_college ON users(college);

-- Create an index on course for faster queries
CREATE INDEX IF NOT EXISTS idx_users_course ON users(course);

-- Create an index on year for faster queries
